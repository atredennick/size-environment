...
make.P.values <- function(v,u,muW,Gpars,Spars,do_year,surv_allsize_years,grow_allsize_years){
  S(u, v , muW, Spars, do_year,surv_allsize_years)*G(u, v, muW, Gpars, do_year,grow_allsize_years) 
}
...
#' @param do_year The year we want to grab for random year effects
#' @return A u*v sized projection matrix
make.K.matrix <- function(v,muW,Rpars,rpa,Gpars,Spars,do_year,surv_allsize_years,grow_allsize_years,fecundity_cutoff) {
  muW <- expandW(v,muW)
  ...
  P.matrix <- outer(v,v,make.P.values,muW,Gpars,Spars,do_year,surv_allsize_years,grow_allsize_years)
