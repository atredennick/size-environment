...
make.P.values=function(v,u,muW, #state variables
                       Gpars,Spars,do_year,G,S,
                       surv_allsize_years,grow_allsize_years)  #growth arguments
{
...
make.P.matrix=function(v,Wvec,Gpars,Spars,do_year=NULL,G,S,surv_allsize_years,grow_allsize_years) {
  muW <- expandW(v,Wvec)
  P.matrix <- outer(v,v,make.P.values,muW,Gpars,Spars,do_year,G,S,surv_allsize_years,grow_allsize_years)
  return(h*P.matrix)
}
