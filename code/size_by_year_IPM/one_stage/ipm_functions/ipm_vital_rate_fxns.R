...
G <- function(u,v,W,Gpars,do_year){
  ...
  mu    <- Gpars$intercept + (Gpars$W)*W + Gpars$logarea.t0*u + (Gpars$logarea.W)*(u*W)
...
S <- function(u,v,W,Spars,do_year){
  ...
  mu <- Spars$intercept + (Spars$W)*W + Spars$logarea*u + (Spars$logarea.W)*(u*W)
...
f <- function(u,v,Rpars,rpa){ 
  nRecruits <-  rpa*exp(u)
  # if(fecundity_cutoff < 100){
  #   max_rec <- nRecruits[fecundity_cutoff]
  #   nRecruits[(fecundity_cutoff+1):length(u)] <- max_rec
  # }
