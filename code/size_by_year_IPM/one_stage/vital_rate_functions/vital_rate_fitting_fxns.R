...
#' @return A list with: (1) lmer model object and (2) fitted variance parameters.
fit_growth <- function(df, size_by_year) {
  ...
  if(size_by_year == TRUE) {
    ...
    out_var <- c(sigma.a=as.numeric(coef(outVar)[1]), sigma.b=as.numeric(coef(outVar)[2]))
...
fit_survival <- function(df, size_by_year) {
  # Require lme4 for GLMMs
  require(lme4)
