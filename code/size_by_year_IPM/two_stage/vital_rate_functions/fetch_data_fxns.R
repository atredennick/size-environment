#' Fetches and formats two-stage growth data.
#' 
...
#' @return A list with data frames: (1) full data, (2) Adult only data, (3) Juvenile only data.
fetch_grow_data <- function(data_dir, state, do_species) {
  ...
  seedling_size <- round(as.numeric(names(sort(table(exp(growdata_full$logarea.t0)),decreasing=TRUE)[1])),5)
  if(seedling_size > 0.26) seedling_size <- 0.26
  growdata_adults <- trim_quadrats(data       = growdata_full, 
                                   skip_years = TRUE, 
                                   juvenile   = FALSE, 
                                   survival   = FALSE,
                                   seedling_size = seedling_size)
  growdata_juvies <- trim_quadrats(data       = growdata_full, 
                                   skip_years = TRUE, 
                                   juvenile   = TRUE, 
                                   survival   = FALSE,
                                   seedling_size = seedling_size)
  
  if(nrow(growdata_adults) + nrow(growdata_juvies) != nrow(growdata_full)) {
    warning("Trimming quadrats lost some observations; check it out.")
  }
  
  return(list(all = growdata_full, adults = growdata_adults, juveniles = growdata_juvies))
}



#' Fetches and formats two-stage survival data.
#' 
#' @author Andrew Tredennick
#' @param data_dir Path to directory containing data.
#' @param state  The state we are working with.
...
fetch_surv_data <- function(data_dir, state, do_species) {
  ...
  seedling_size <- round(as.numeric(names(sort(table(exp(survdata_full$logarea)),decreasing=TRUE)[1])),5)
  if(seedling_size > 0.26) seedling_size <- 0.26
  survdata_adults <- trim_quadrats(data       = survdata_full, 
                                   skip_years = TRUE, 
                                   juvenile   = FALSE, 
                                   survival   = TRUE,
                                   seedling_size = seedling_size)
  survdata_juvies <- trim_quadrats(data       = survdata_full, 
                                   skip_years = TRUE, 
                                   juvenile   = TRUE, 
                                   survival   = TRUE,
                                   seedling_size = seedling_size)
  
  if(nrow(survdata_adults) + nrow(survdata_juvies) != nrow(survdata_full)) {
    warning("Trimming quadrats lost some observations; check it out.")
  }
  
  return(list(all = survdata_full, adults = survdata_adults, juveniles = survdata_juvies))
}
