...
#' @param model_fit Path to directory containing data. 
...
#' @author Andrew Tredennick
#' @param model_fit Path to directory containing data. 
#' @param juveniles Logical for juvenile model (TRUE) or adult model (FALSE)
#' @param all_years A vector of year ids for all the years present in the data.
#' @return A dataframe with coefficients for each year.
format_surv_params <- function(model_fit, juveniles, all_years) {
  require(tidyverse)
  require(dplyr)
  ...
  if(juveniles == TRUE){
    ...
    years_not_in <- all_years[which(!(all_years %in% model_params$year_id))]
    model_means <- data.frame(year_id = years_not_in) %>%
      mutate(year = 1900 + year_id)
    model_means$intercept <- fixef(model_fit)[1]
    model_means$W = fixef(model_fit)[2]
    model_means <- model_means %>%
      dplyr::select(intercept, W, year_id, year)
    
    model_params <- rbind(model_params, model_means)
  }
