...
trim_quadrats <- function(data, skip_years=TRUE, juvenile, survival, seedling_size) {  
  
  if(survival==TRUE){data$logarea.t0=data$logarea}
  ...
  for(j in 1:length(quadrats)) {
    ...
    if(skip_years){ # trim dubious ages after a gap in the data for a quadrat
  ...
  if(juvenile==FALSE){
    if(length(likely.seedlings)>0) data=data[-(likely.seedlings), ]; 
  }
