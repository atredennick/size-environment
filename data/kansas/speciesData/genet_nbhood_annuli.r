...
focalSpp<-paste0("h:/hayschart/ipm2/Age & Size2/speciesData/",focalSppName,"/",focalSppName,"_genet_xy.csv")
sppShapes <- c("h:/hayschart/lifetables/polys/Species/Andropogon gerardii/",
               "h:/hayschart/lifetables/polys/Species/Bouteloua curtipendula/",
               "h:/hayschart/lifetables/polys/Species/Bouteloua hirsuta/",
                "h:/hayschart/lifetables/polys/Species/Schizachyrium scoparium/")
nbSpp <- c("ANGE","BOCU","BOHI","SCSC")  # make sure these are listed in same order as sppShapes
annuli <- c(seq(2,20,2),seq(25,50,5),seq(60,150,10))
outfile <- paste0("h:/hayschart/ipm2/Age & Size2/speciesData/",focalSppName,"/",focalSppName,"_nbhood_rings.csv")
...
for(i in 1:NROW(quadyear)){
  ...
  for(j in 1:length(sppShapes)){
    ...
    tmpfile<-paste(doQuad,"-",doYear,".shp",sep="" )
