...
focalSpp<-paste0("h:/montanachart/ipm/speciesData/",focalSppName,"/edited/",focalSppName,"_genet_xy_edited.csv")
sppShapes <- c("h:/montanachart/lifetables/polys/Species/BOGR-edited/Bouteloua gracilis/",
...
# loop through focal genets
  for(k in 1:NROW(tmpD)){
    ...
    print(list(i=paste(i,"out of", NROW(quadyear), sep=" "),k=paste(k, "out of", NROW(tmpD), sep=" "),
               doQuad=doQuad, doYear=doYear, ID=ID, x=x, y=y))
