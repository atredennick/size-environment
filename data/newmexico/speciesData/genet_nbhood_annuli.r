...
focalSpp<-paste0("h:/newmexicochart/ipm/Age & Size2/speciesData/",focalSppName,"/",focalSppName,"_genet_xy.csv")
sppShapes <- c("h:/newmexicochart/lifetables/polys/Species/Bouteloua eriopoda/",
               "h:/newmexicochart/lifetables/polys/Species/Gutierrezia sarothrae/",
               "h:/newmexicochart/lifetables/polys/Species/Scleropogon brevifolius/",
                "h:/newmexicochart/lifetables/polys/Species/Sporobolus flexuosus/")
nbSpp <- c("BOER","GUSA","SCBR","SPFL")  # make sure these are listed in same order as sppShapes
annuli <- c(seq(2,20,2),seq(25,50,5),seq(60,150,10))
outfile <- paste0("h:/newmexicochart/ipm/Age & Size2/speciesData/",focalSppName,"/",focalSppName,"_nbhood_rings.csv")
...
for(i in 1:NROW(quadyear)){
  ...
  for(j in 1:length(sppShapes)){
    ...
    listing<-list.files(tmpDir)
    shpfiles<-grep((".shp"),listing, value=TRUE)
    tmpfile<-grep(paste0(doQuad,"_",doYear), shpfiles, value=TRUE)
